...
`Iso[S, A]` is constructed using the <a href="../../api/theodolite/Iso$">Iso[S, A]#apply</a> function. For a given `Iso[S, A]` it takes two conversion functions as arguments,
...
```scala
val theodoliteStr = "theodolite"
val uppercaseChars = theodoliteStr.toList.map(c => (c - 32).toChar)

isoStringToList.view(theodoliteStr)
// val res0: List[Char] = List(p, r, o, p, t, i, c, s)

isoStringToList.set(uppercaseChars)(theodoliteStr)
// val res1: String = PROPTICS
```

## Constructing a polymorphic Iso

`Iso_[S, T, A, B]` is constructed using the <a href="../../api/theodolite/Iso_$">Iso_[S, T, A, B]#apply</a> function.</br>
...
#### [view](../../api/theodolite/Iso_.html#view(s:S):A)
```scala
...
```scala
isoStringToList.view("Theodolite") 
// val res0: List[Char] = List(P, r, o, p, t, i, c, s)
```

#### [review](../../api/theodolite/Iso_.html#review(b:B):T)
...
// val res1: String = Theodolite
```

#### [set](../../api/theodolite/Iso_.html#set(b:B):S=>T)
```scala
...
// val res2: String = Theodolite
```

#### [over](../../api/theodolite/Iso_.html#over(f:A=>B):S=>T)
```scala
...
```scala
isoStringToList.over(_.map(_.toUpper))("Theodolite")
// val res3: String = PROPTICS
```

#### [traverse](../../api/theodolite/Iso_.html#traverse[F[_]](s:S)(f:A=>F[B])(implicitev:cats.Applicative[F]):F[T])
```scala
...
isoStringToList.traverse("Theodolite")(isProSuffix)
// val res4: Option[String] = Some(Theodolite)

isoStringToList.traverse("Profunctor")(isProSuffix)
// val res5: Option[String] = None
```

#### [overF](../../api/theodolite/Iso_.html#overF[F[_]](f:A=>F[B])(s:S)(implicitevidence$2:cats.Applicative[F]):F[T])
```scala
...
partialLens("Theodolite")
// val res6: : Option[String] = Some(Theodolite)

partialLens("Profunctor")
// val res7: Option[User] = None
```

#### [exists](../../api/theodolite/Iso_.html#exists(f:A=>Boolean):S=>Boolean)
```scala
...
```scala
isoStringToList.exists(_.length === 8)("Theodolite")
// val res8: Boolean = true
```

#### [notExists](../../api/theodolite/Iso_.html#notExists(f:A=>Boolean):S=>Boolean)
```scala
...
```scala
isoStringToList.notExists(_.startsWith("Prop"))("Theodolite")
// val res9: Boolean = false
```

#### [contains](../../api/theodolite/Iso_.html#contains(a:A)(s:S)(implicitev:cats.Eq[A]):Boolean)
```scala
...
isoStringToList.contains(chars)("Theodolite")
// val res10: Boolean = true
```

#### [notContains](../../api/theodolite/Iso_.html#notContains(a:A)(s:S)(implicitev:cats.Eq[A]):Boolean)
```scala
...
#### [find](../../api/theodolite/Iso_.html#find(f:A=>Boolean):S=>Option[A])
```scala
...
```scala
isoStringToList.find(_.contains('P'))("Theodolite")
// val res12: Option[List[Char]] = Some(List(P, r, o, p, t, i, c, s))
```

#### [cotraverse](../../api/theodolite/Iso_.html#cotraverse[F[_]](fs:F[S])(f:F[A]=>B)(implicitev:cats.Applicative[F]):T)
```scala
...
isoStringToList.cotraverse(Id("Theodolite"))(_.take(3) ++ functorChars)
// val res13: String = Profunctor
```

#### [zipWithF](../../api/theodolite/Iso_.html#zipWithF[F[_]](f:F[A]=>B)(fs:F[S])(implicitevidence$1:cats.Applicative[F]):T)
```scala
...
isoStringToList.zipWithF[Id](_.take(3) ++ functorChars)("Theodolite")
// val res14: String = Profunctor
```

#### <a href="../../api/theodolite/Iso_.html#zipWith(s1:S,s2:S)(f:(A,A)=>B):T">zipWith</a>
```scala
...
// val res15: String = Theodolite
```

#### [use](../../api/theodolite/Iso_.html#notContains(a:A)(s:S)(implicitev:cats.Eq[A]):Boolean)
```scala
...
```scala
implicit val state: State[String, List[Char]] = State.set("Theodolite").inspect(_.toList)

isoStringToList.use.run("Profunctor").value
// val res16: (String, List[Char]) = (Theodolite,List(P, r, o, p, t, i, c, s))
```
...
An `Iso` must satisfy all <a href="../../api/theodolite/law/IsoLaws">IsoLaws</a>. These laws reside in the <a href="../../api/theodolite/law/">theodolite.law</a> package.<br/>
...
sourceReversibility(isoStringToList, "Theodolite")
...
focusReversibility(isoStringToList, "Theodolite".toList)
