...
import theodolite.{APrism, Prism}
...
private[macros] class GenPrismMacro(val c: blackbox.Context) {
  ...
  def genPrism[Prism: c.WeakTypeTag, S: c.WeakTypeTag, A: c.WeakTypeTag]: c.Expr[Prism] = {
    ...
    val prismTree =
      q"""  
          _root_.theodolite.$prismName.fromPartial[$typeOfS, $typeOfA] { case s if s.isInstanceOf[$typeOfA] => s.asInstanceOf[$typeOfA] }(_.asInstanceOf[$typeOfS])
       """
