...
import theodolite.data.{Additive, Disj, Dual, Endo, Last, Multiplicative}
import theodolite.syntax.function._

private[theodolite] trait Fold1[S, A] extends Fold0[S, A] {
  /** synonym for [[fold]] */
  ...
  protected[theodolite] def minMax(s: S)(f: (A, A) => A): Option[A] =
