...
import theodolite.Lens_
import theodolite.internal.partsOf.{ins, outs, unsafeOuts}
import theodolite.profunctor.Corepresentable.Aux
import theodolite.profunctor.{Choice, Traversing, Wander}
import theodolite.rank2types.Rank2TypeLensLike

/** Bazaar is used to characterize a [[theodolite.Traversal_]] */
...
object Bazaar extends BazaarInstances {
  private[theodolite] def partsOf[S, T, A](bazaar: Bazaar[* => *, A, A, S, T])(implicit ev0: Sellable[* => *, Bazaar[* => *, *, *, Unit, *]]): Lens_[S, T, List[A], List[A]] =
  ...
  private[theodolite] def unsafePartsOf[S, T, A, B](
