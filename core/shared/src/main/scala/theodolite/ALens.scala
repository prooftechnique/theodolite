...
/** An [[ALens_]] ] focuses a single piece of data within a larger structure.
  ...
  *
  * An [[ALens_]] is a [[Lens_]] with fixed type [[theodolite.internal.Shop]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class ALens_[S, T, A, B] extends Lens0[S, T, A, B] { self =>
  private[theodolite] def apply(shop: Shop[A, B, A, B]): Shop[A, B, S, T]
  ...
  final def compose[C, D](other: Lens_[C, D, S, T]): ALens_[C, D, A, B] = new ALens_[C, D, A, B] {
    override private[theodolite] def apply(shop: Shop[A, B, A, B]): Shop[A, B, C, D] =
  ...
  final def compose[C, D](other: ALens_[C, D, S, T]): ALens_[C, D, A, B] = new ALens_[C, D, A, B] {
    override private[theodolite] def apply(shop: Shop[A, B, A, B]): Shop[A, B, C, D] = other.toShop andThen self(shop)
  }
  ...
  final def andThen[C, D](other: Setter_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    final override private[theodolite] def apply(pab: C => D): S => T = s => self.set(other(pab)(self.view(s)))(s)
  }
  ...
  final def andThen[C, D](other: Getter_[A, B, C, D]): Getter_[S, T, C, D] = new Getter_[S, T, C, D] {
    final override private[theodolite] def apply(forget: Forget[C, C, D]): Forget[C, S, T] =
  ...
  final def andThen[I, C, D](other: IndexedSetter_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    final override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T = s => self.set(other.over { case (c, i) => indexed.runIndex((c, i)) }(self.view(s)))(s)
  }
...
object ALens_ {
  ...
  final private[theodolite] def apply[S, T, A, B](f: Shop[A, B, A, B] => Shop[A, B, S, T]): ALens_[S, T, A, B] = new ALens_[S, T, A, B] { self =>
