...
import theodolite.profunctor.{Choice, Star}
...
/** [[APrism_]] is used for selecting cases of a type, most often a sum type.
  *
  * [[APrism_]] is a [[Prism_]] with fixed type [[theodolite.internal.Market]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class APrism_[S, T, A, B] extends Prism0[S, T, A, B] { self =>
  private[theodolite] def apply(market: Market[A, B, A, B]): Market[A, B, S, T]
  ...
  final def andThen[C, D](other: APrism_[A, B, C, D]): APrism_[S, T, C, D] = new APrism_[S, T, C, D] {
    override private[theodolite] def apply(market: Market[C, D, C, D]): Market[C, D, S, T] = self.toMarket andThen other(market)
  ...
  final def compose[C, D](other: APrism_[C, D, S, T]): APrism_[C, D, A, B] = new APrism_[C, D, A, B] {
    override private[theodolite] def apply(market: Market[A, B, A, B]): Market[A, B, C, D] = other.toMarket andThen self(market)
  ...
  final def andThen[C, D](other: Setter_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T =
  ...
  final def compose[C, D](other: Setter_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D =
...
object APrism_ {
  ...
  final def apply[S, T, A, B](_viewOrModify: S => Either[T, A])(_review: B => T): APrism_[S, T, A, B] = new APrism_[S, T, A, B] { self =>
    override private[theodolite] def apply(market: Market[A, B, A, B]): Market[A, B, S, T] = Market(_viewOrModify, _review)
