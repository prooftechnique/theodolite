...
/** [[AnAffineTraversal_]] has at most one focus, but is not a [[Prism_]], you cannot use it to construct a value.
  *
  * [[AnAffineTraversal_]] is an [[AffineTraversal_]] with fixed type [[theodolite.internal.Stall]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class AnAffineTraversal_[S, T, A, B] extends AffineTraversal0[S, T, A, B] { self =>
  private[theodolite] def apply(pab: Stall[A, B, A, B]): Stall[A, B, S, T]
  ...
  /** convert an [[AnAffineTraversal_]] to an [[theodolite.internal.Stall]] */
...
object AnAffineTraversal_ {
  /** create a polymorphic [[AnAffineTraversal_]] from an [[AnAffineTraversal_]] encoded in [[theodolite.internal.Stall]] */
  private[theodolite] def apply[S, T, A, B](f: Stall[A, B, A, B] => Stall[A, B, S, T]): AnAffineTraversal_[S, T, A, B] = new AnAffineTraversal_[S, T, A, B] { self =>
