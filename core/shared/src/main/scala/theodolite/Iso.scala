...
import theodolite.internal.Forget._
import theodolite.internal._
import theodolite.profunctor.{Choice, Closed, Costar, Wander}
import theodolite.rank2types._
import theodolite.syntax.costar._
...
abstract class Iso_[S, T, A, B] extends Iso0[S, T, A, B] { self =>
  private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Profunctor[P]): P[S, T]
  ...
  final def compose[C, D](other: Iso_[C, D, S, T]): Iso_[C, D, A, B] = new Iso_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Profunctor[P]): P[C, D] = other(self(pab))
  }
  ...
  final def andThen[C, D](other: AnIso_[A, B, C, D]): AnIso_[S, T, C, D] = new AnIso_[S, T, C, D] {
    override private[theodolite] def apply(exchange: Exchange[C, D, C, D]): Exchange[C, D, S, T] = self(other(exchange))
  ...
  final def compose[C, D](other: AffineTraversal_[C, D, S, T]): AffineTraversal_[C, D, A, B] = new AffineTraversal_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev0: Choice[P], ev1: Strong[P]): P[C, D] =
      other(self(pab)(ev1))
  ...
  final def compose[C, D](other: Grate_[C, D, S, T]): Grate_[C, D, A, B] = new Grate_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Closed[P]): P[C, D] = other(self(pab))
  }
  ...
  final def andThen[I, C, D](other: IndexedSetter_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T = s => self.set(other.over(indexed.runIndex)(self.view(s)))(s)
  }
...
object Iso_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Rank2TypeIsoLike[S, T, A, B]): Iso_[S, T, A, B] = new Iso_[S, T, A, B] {
...
object Iso {
  ...
  private[theodolite] def apply[S, A](f: Rank2TypeIsoLike[S, S, A, A]): Iso[S, A] = new Iso[S, A] {
