...
import theodolite.IndexedLens_.liftIndexedOptic
...
/** An [[AnIndexedLens_]] ] focuses a single piece of data and index within a larger structure.
  ...
  *
  * an [[AnIndexedLens_]] is an [[IndexedLens_]] with fixed type [[theodolite.internal.Shop]] [[cats.arrow.Profunctor]]
  *
  * @tparam I
  ...
  */
abstract class AnIndexedLens_[I, S, T, A, B] extends IndexedLens0[I, S, T, A, B] { self =>
  private[theodolite] def apply(indexed: Indexed[Shop[(A, I), B, *, *], I, A, B]): Shop[(A, I), B, S, T]

  /** view the focus and the index of an [[AnIndexedLens_]] */
  final override def view(s: S): (A, I) = toShop.view(s)
  ...
  final def andThen[C, D](other: Setter_[A, B, C, D]): Setter_[S, T, (C, I), D] = new Setter_[S, T, (C, I), D] {
    override private[theodolite] def apply(pab: ((C, I)) => D): S => T =
  ...
  final def compose[C, D](other: Setter_[C, D, S, T]): Setter_[C, D, (A, I), B] = new Setter_[C, D, (A, I), B] {
    override private[theodolite] def apply(pab: ((A, I)) => B): C => D =
  ...
  final def andThen[C, D](other: Getter_[A, B, C, D]): Getter_[S, T, (C, I), D] = new Getter_[S, T, (C, I), D] {
    override private[theodolite] def apply(forget: Forget[(C, I), (C, I), D]): Forget[(C, I), S, T] =
      Forget { s =>
        val (a, i) = self.view(s)
  ...
  final def compose[C, D](other: Getter_[C, D, S, T]): Getter_[C, D, (A, I), B] = new Getter_[C, D, (A, I), B] {
    override private[theodolite] def apply(forget: Forget[(A, I), (A, I), B]): Forget[(A, I), C, D] =
      Forget { c =>
  ...
  final def andThen[C, D](other: Fold_[A, B, C, D]): Fold_[S, T, (C, I), D] = new Fold_[S, T, (C, I), D] {
    override private[theodolite] def apply[R: Monoid](forget: Forget[R, (C, I), D]): Forget[R, S, T] =
      Forget { s =>
        val (a, i) = self.view(s)
  ...
  final def compose[C, D](other: Fold_[C, D, S, T]): Fold_[C, D, (A, I), B] = new Fold_[C, D, (A, I), B] {
    override private[theodolite] def apply[R: Monoid](forget: Forget[R, (A, I), B]): Forget[R, C, D] =
  ...
  final def andThenWithRightIndex[J, C, D](other: IndexedSetter_[J, A, B, C, D]): IndexedSetter_[J, S, T, C, D] = new IndexedSetter_[J, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, J, C, D]): S => T = s => self.set(other.over(indexed.runIndex)(self.view(s)._1))(s)
  }
  ...
  final def andThenWithLeftIndex[J, C, D](other: IndexedSetter_[_, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T = s => {
      val (a, i) = self.view(s)
...
object AnIndexedLens_ {
  ...
  private[theodolite] def apply[I, S, T, A, B](f: Rank2TypeIndexedLensLike[I, S, T, A, B]): AnIndexedLens_[I, S, T, A, B] = new AnIndexedLens_[I, S, T, A, B] {
