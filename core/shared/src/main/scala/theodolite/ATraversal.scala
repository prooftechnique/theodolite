...
import theodolite.rank2types.LensLikeWithIndex
import theodolite.syntax.aTraversal._

/** [[ATraversal_]] is an optic that focuses on zero or more values.
  *
  * [[ATraversal_]] is a [[Traversal_]] with fixed type [[theodolite.internal.Bazaar]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class ATraversal_[S, T, A, B] extends Traversal1[S, T, A, B] { self =>
  private[theodolite] def apply(bazaar: Bazaar[* => *, A, B, A, B]): Bazaar[* => *, A, B, S, T]
  ...
  /** convert an [[ATraversal_]] to a [[theodolite.internal.Bazaar]] */
  ...
  final def asTraversal: Traversal_[S, T, A, B] = new Traversal_[S, T, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Wander[P]): P[S, T] = {
...
object ATraversal_ {
  ...
  private[theodolite] def apply[S, T, A, B](runBazaar0: RunBazaar[* => *, A, B, S, T]): ATraversal_[S, T, A, B] = new ATraversal_[S, T, A, B] {
    override private[theodolite] def apply(bazaar: Bazaar[Function, A, B, A, B]): Bazaar[Function, A, B, S, T] = new Bazaar[* => *, A, B, S, T] {
  ...
  /** create a polymorphic [[ATraversal_]] from [[theodolite.internal.Bazaar]] */
...
object ATraversal {
  ...
  /** create a monomorphic [[ATraversal]] from a [[theodolite.internal.Bazaar]] */
