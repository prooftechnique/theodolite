package theodolite.applied

import theodolite._
import theodolite.applied.internal.AppliedFold1
...
trait AppliedFold_[S, T, A, B] extends AppliedFold1[S, A] {
  ...
  final def focus[C](f: A => C): AppliedFold_[S, T, C, C] = AppliedFold_(value, optic.andThen(Getter_[A, B, C, C](f)))

  /** compose this [[Fold_]] with an [[AnIso_]], having this [[Fold_]] applied first */
  final def andThen[C, D](other: Iso_[A, B, C, D]): AppliedFold_[S, T, C, D] = AppliedFold_(value, optic.andThen(other))

  /** compose this [[Fold_]] with an [[AnIso_]], having this [[Fold_]] applied first */
  final def andThen[C, D](other: AnIso_[A, B, C, D]): AppliedFold_[S, T, C, D] = AppliedFold_(value, optic.andThen(other))

  /** compose this [[Fold_]] with a [[Lens_]], having this [[Fold_]] applied first */
  final def andThen[C, D](other: Lens_[A, B, C, D]): AppliedFold_[S, T, C, D] = AppliedFold_(value, optic.andThen(other))

  /** compose this [[Fold_]] with an [[ALens_]], having this [[Fold_]] applied first */
  final def andThen[C, D](other: ALens_[A, B, C, D]): AppliedFold_[S, T, C, D] = AppliedFold_(value, optic.andThen(other))
