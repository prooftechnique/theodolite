...
import theodolite.profunctor.{Choice, Costar, Star, Wander}
import theodolite.rank2types.{LensLikeWithIndex, Rank2TypeLensLike}
import theodolite.syntax.costar._
import theodolite.syntax.function._
...
abstract class Lens_[S, T, A, B] extends Lens0[S, T, A, B] { self =>
  private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Strong[P]): P[S, T]
  ...
  final def andThen[C, D](other: Iso_[A, B, C, D]): Lens_[S, T, C, D] = new Lens_[S, T, C, D] {
    override private[theodolite] def apply[P[_, _]](pab: P[C, D])(implicit ev: Strong[P]): P[S, T] = self(other(pab))
  }

  /** compose this [[Lens_]] with an [[Iso_]], having this [[Lens_]] applied last */
  final def compose[C, D](other: Iso_[C, D, S, T]): Lens_[C, D, A, B] = new Lens_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Strong[P]): P[C, D] = other(self(pab))
  }
  ...
  final def andThen[C, D](other: Getter_[A, B, C, D]): Getter_[S, T, C, D] = new Getter_[S, T, C, D] {
    override private[theodolite] def apply(forget: Forget[C, C, D]): Forget[C, S, T] = self(other(Forget(identity)))
  }
  ...
  final def andThen[I, C, D](other: IndexedSetter_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T = s => self.set(other.over { case (c, i) => indexed.runIndex((c, i)) }(self.view(s)))(s)
  }
...
object Lens_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Rank2TypeLensLike[S, T, A, B]): Lens_[S, T, A, B] = new Lens_[S, T, A, B] { self =>
  ...
  private[theodolite] def liftOptic[P[_, _], S, T, A, B](to: S => (A, B => T))(implicit ev: Strong[P]): P[A, B] => P[S, T] =
