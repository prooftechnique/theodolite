...
import theodolite._
import theodolite.applied.{AppliedFold_, AppliedLens_, AppliedTraversal_}
import theodolite.std.list._
import theodolite.std.string.{mkString => mkStr, takeWords => tkWords, _}
import theodolite.std.tuple.{_1P, _2P}
...
final case class Tuple2ToPolyAppliedLensOps[A, B](private val s: (A, B)) extends AnyVal {
  /** select the first element of a tuple using polymorphic [[theodolite.Lens_]] */
  def first_[C]: AppliedLens_[(A, B), (C, B), A, C] = AppliedLens_(s, _1P[A, C, B])

  /** select the second element of a tuple using polymorphic [[theodolite.Lens_]] */
...
final case class AppliedLensTraversalOps[F[_], S, T, A](private val appliedLens: AppliedLens_[S, T, F[A], F[A]]) extends AnyVal {
  ...
  /** compose this [[AppliedLens_]] with a [[theodolite.Fold]] having this [[AppliedLens_]] applied first */
