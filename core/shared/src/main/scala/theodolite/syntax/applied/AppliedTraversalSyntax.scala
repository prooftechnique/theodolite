...
import theodolite.applied.{AppliedLens_, AppliedTraversal_}
import theodolite.internal.{Bazaar, Sellable}
import theodolite.std.list._
import theodolite.std.string.{mkString => mkStr, takeWords => tkWords, _}
import theodolite.syntax.traversal._
import theodolite.{AppliedTraversal, Traversal, Traversal_}
...
final case class AppliedTraversalElementOps[S, T, A](private val appliedTraversal: AppliedTraversal_[S, T, A, A]) extends AnyVal {
  ...
  /** convert a [[Traversal_]] into a [[theodolite.Lens]] over a list of the Traversal's foci */
...
final case class AppliedBitraversalElementOps[G[_, _], A](private val s: G[A, A]) extends AnyVal {
  /** create a polymorphic [[theodolite.Traversal_]] for structure `G[A, A]` that can traverse both sides */
  def bitraverse_[B](implicit ev: Bitraverse[G]): AppliedTraversal_[G[A, A], G[B, B], A, B] =
    AppliedTraversal_(s, Traversal_.both[G, A, B])

  /** create a monomorphic [[theodolite.Traversal_]] for structure `G[A, A]` that can traverse both sides */
