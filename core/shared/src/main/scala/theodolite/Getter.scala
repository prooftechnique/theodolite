...
import theodolite.internal.{Forget, Getter1, Indexed}
...
abstract class Getter_[S, T, A, B] extends Getter1[S, A] { self =>
  private[theodolite] def apply(forget: Forget[A, A, B]): Forget[A, S, T]
  ...
  final def andThen[I, C, D](other: IndexedLens_[I, A, B, C, D]): IndexedGetter_[I, S, T, C, D] = new IndexedGetter_[I, S, T, C, D] {
    private[theodolite] def apply(indexed: Indexed[Forget[(C, I), *, *], I, C, D]): Forget[(C, I), S, T] =
