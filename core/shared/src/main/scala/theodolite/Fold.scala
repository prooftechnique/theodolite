...
import theodolite.internal.{Fold1, Forget, Indexed}
import theodolite.rank2types.{Rank2TypeFoldLike, Rank2TypeIndexedFoldLike}
import theodolite.syntax.fold._
...
/** A [[Fold_]] is a generalization of something Foldable. It describes how to retrieve multiple values.
  ...
  *
  * A [[Fold_]] is an Optic with fixed type [[theodolite.internal.Forget]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class Fold_[S, T, A, B] extends Fold1[S, A] { self =>
  private[theodolite] def apply[R: Monoid](forget: Forget[R, A, B]): Forget[R, S, T]
...
object Fold_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Rank2TypeFoldLike[S, T, A, B]): Fold_[S, T, A, B] = new Fold_[S, T, A, B] { self =>
  ...
  final def fromFoldable[F[_], A, B](implicit ev0: Foldable[F]): Fold_[F[A], F[B], A, B] = new Fold_[F[A], F[B], A, B] {
    override private[theodolite] def apply[R: Monoid](forget: Forget[R, A, B]): Forget[R, F[A], F[B]] =
  ...
  private[theodolite] def fromGetRank2TypeFoldLike[S, T, A, B](get: S => A): Rank2TypeFoldLike[S, T, A, B] = new Rank2TypeFoldLike[S, T, A, B] {
    override def apply[R: Monoid](forget: Forget[R, A, B]): Forget[R, S, T] = liftForget[R, S, T, A, B](get)(forget)
  }

  private[theodolite] def liftForget[R, S, T, A, B](f: S => A): Forget[R, A, B] => Forget[R, S, T] =
