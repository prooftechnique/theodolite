...
import theodolite.internal.{Indexed, Setter0}
...
abstract class Setter_[S, T, A, B] extends Setter0[S, T, A, B] { self =>
  private[theodolite] def apply(pab: A => B): S => T
  ...
  final def andThen[C, D](other: Iso_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with an [[Iso_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: Iso_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other(self(pab))
  }

  /** compose this [[Setter_]] with an [[AnIso_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: AnIso_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self.over(other.over(pab))
  }

  /** compose this [[Setter_]] with an [[AnIso_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: AnIso_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with a [[Lens_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: Lens_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with a [[Lens_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: Lens_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with an [[ALens_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: ALens_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other.over(pab))
  }

  /** compose this [[Setter_]] with an [[ALens_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: ALens_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with a [[Prism_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: Prism_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with a [[Prism_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: Prism_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with an [[APrism_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: APrism_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self.over(other.over(pab))
  }

  /** compose this [[Setter_]] with an [[APrism_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: APrism_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with a [[AffineTraversal_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: AffineTraversal_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with a [[AffineTraversal_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: AffineTraversal_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with an [[AnAffineTraversal_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: AnAffineTraversal_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other.over(pab))
  }

  /** compose this [[Setter_]] with an [[AnAffineTraversal_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: AnAffineTraversal_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with a [[Traversal_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: Traversal_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with a [[Traversal_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: Traversal_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with an [[ATraversal_]], having this [[Setter_]] applied first */
  final def andThen[C, D](other: ATraversal_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other.over(pab))
  }

  /** compose this [[Setter_]] with an [[ATraversal_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: ATraversal_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }
  ...
  final def andThen[C, D](other: Grate_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }

  /** compose this [[Setter_]] with a [[Grate_]], having this [[Setter_]] applied last */
  final def compose[C, D](other: Grate_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other.over(self.over(pab))
  }

  /** compose this [[Setter_]] with an [[IndexedLens_]], having this [[Setter_]] applied first */
  final def andThen[I, C, D](other: IndexedLens_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T =
      self.over(other.over(indexed.runIndex))
  }

  /** compose this [[Setter_]] with an [[IndexedLens_]], having this [[Setter_]] applied last */
  final def compose[I, C, D](other: IndexedLens_[I, C, D, S, T]): IndexedSetter_[I, C, D, A, B] = new IndexedSetter_[I, C, D, A, B] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, A, B]): C => D =
  ...
  final def andThen[I, C, D](other: AnIndexedLens_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T =
      self.over(other.over(indexed.runIndex))
  }

  /** compose this [[Setter_]] with an [[AnIndexedLens_]], having this [[Setter_]] applied last */
  final def compose[I, C, D](other: AnIndexedLens_[I, C, D, S, T]): IndexedSetter_[I, C, D, A, B] = new IndexedSetter_[I, C, D, A, B] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, A, B]): C => D =
      other.over { case (s, i) => self.over(a => indexed.runIndex((a, i)))(s) }
  }

  /** compose this [[Setter_]] with an [[IndexedTraversal_]], having this [[Setter_]] applied first */
  final def andThen[I, C, D](other: IndexedTraversal_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T =
      self(other(Indexed(indexed.runIndex)))
  }

  /** compose this [[Setter_]] with an [[IndexedTraversal_]], having this [[Setter_]] applied last */
  final def compose[I, C, D](other: IndexedTraversal_[I, C, D, S, T]): IndexedSetter_[I, C, D, A, B] = new IndexedSetter_[I, C, D, A, B] {
    override private[theodolite] def apply(indexed: Indexed[* => *, I, A, B]): C => D =
      other.over { case (s, i) => self.over(a => indexed.runIndex((a, i)))(s) }
  }
