...
import theodolite.internal.{Indexed, IndexedSetter0}
import theodolite.syntax.tuple._

/** An [[IndexedSetter_]] is a generalization of mapWithIndex from [[theodolite.indices.FunctorWithIndex]]
  *
  * @tparam I
...
abstract class IndexedSetter_[I, S, T, A, B] extends IndexedSetter0[I, S, T, A, B] { self =>
  private[theodolite] def apply(indexed: Indexed[* => *, I, A, B]): S => T
  ...
  final def reindex[J](f: I => J): IndexedSetter_[J, S, T, A, B] = new IndexedSetter_[J, S, T, A, B] {
    override private[theodolite] def apply(indexed: Indexed[* => *, J, A, B]): S => T = self(indexed.reindex[I](f))
  }

  /** transform an [[IndexedSetter_]] to a [[Setter_]] */
  final def asSetter: Setter_[S, T, A, B] = new Setter_[S, T, A, B] {
    override private[theodolite] def apply(pab: A => B): S => T =
...
object IndexedSetter_ {
  ...
  private[theodolite] def apply[I, S, T, A, B](mapping: Indexed[* => *, I, A, B] => S => T)(implicit ev: DummyImplicit): IndexedSetter_[I, S, T, A, B] =
