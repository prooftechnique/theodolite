...
import theodolite.internal.{Review0, Tagged}
...
abstract class Review_[S, T, A, B] extends Review0[T, B] { self =>
  private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[S, T]
  ...
  final def andThen[C, D](other: Iso_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
    override private[theodolite] def apply(tagged: Tagged[C, D]): Tagged[S, T] = self(other(tagged)(Tagged.profunctorTagged))
  }

  /** compose this [[Review_]] with an [[Iso_]], having this [[Review_]] applied last */
  final def compose[C, D](other: Iso_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
  ...
  final def andThen[C, D](other: AnIso_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
  ...
  final def compose[C, D](other: AnIso_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
    override private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[C, D] =
      Tagged(other.review(self.review(tagged.runTag)))
  }

  /** compose this [[Review_]] with a [[Prism_]], having this [[Review_]] applied first */
  final def andThen[C, D](other: Prism_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
  ...
  final def compose[C, D](other: Prism_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
    override private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[C, D] =
      Tagged(other.review(self.review(tagged.runTag)))
  }

  /** compose this [[Review_]] with an [[APrism_]], having this [[Review_]] applied first */
  final def andThen[C, D](other: APrism_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
    override private[theodolite] def apply(tagged: Tagged[C, D]): Tagged[S, T] =
      Tagged(self.review(other.review(tagged.runTag)))
  }

  /** compose this [[Review_]] with an [[APrism_]], having this [[Review_]] applied last */
  final def compose[C, D](other: APrism_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
    override private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[C, D] =
      Tagged(other.review(self.review(tagged.runTag)))
  }

  /** compose this [[Review_]] with a [[Grate_]], having this [[Review_]] applied first */
  final def andThen[C, D](other: Grate_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
    override private[theodolite] def apply(tagged: Tagged[C, D]): Tagged[S, T] = self(other(tagged))
  }

  /** compose this [[Review_]] with a [[Grate_]], having this [[Review_]] applied last */
  final def compose[C, D](other: Grate_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
    override private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[C, D] =
      Tagged(other.review(self.review(tagged.runTag)))
  }
...
object Review_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Tagged[A, B] => Tagged[S, T])(implicit ev: DummyImplicit): Review_[S, T, A, B] =
    new Review_[S, T, A, B] {
