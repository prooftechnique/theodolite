...
import theodolite.internal.{Indexed, Tagged, Zipping}
import theodolite.profunctor.Costar._
import theodolite.profunctor.{Closed, Costar}
import theodolite.rank2types.Rank2TypeGrateLike
...
abstract class Grate_[S, T, A, B] extends Serializable { self =>
  private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Closed[P]): P[S, T]
...
object Grate_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Rank2TypeGrateLike[S, T, A, B]): Grate_[S, T, A, B] = new Grate_[S, T, A, B] {
