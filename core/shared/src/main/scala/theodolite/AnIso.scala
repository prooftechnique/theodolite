...
import theodolite.profunctor.{Choice, Closed, Wander}
import theodolite.rank2types.{LensLike, LensLikeWithIndex}

/** An [[AnIso_]] is a generalized isomorphism.
  *
  * An [[AnIso_]] is a complete reversible transformation between two types.
  *
  * An [[AnIso_]] is an [[Iso_]] with fixed type [[theodolite.internal.Exchange]] [[cats.arrow.Profunctor]]
  *
  * @tparam S
...
abstract class AnIso_[S, T, A, B] extends Iso0[S, T, A, B] { self =>
  private[theodolite] def apply(exchange: Exchange[A, B, A, B]): Exchange[A, B, S, T]
  ...
  final def reverse: AnIso_[B, A, T, S] = new AnIso_[B, A, T, S] {
    override private[theodolite] def apply(exchange: Exchange[T, S, T, S]): Exchange[T, S, B, A] = Exchange(self.review, self.view)
  ...
  final def andThen[C, D](other: Iso_[A, B, C, D]): AnIso_[S, T, C, D] = new AnIso_[S, T, C, D] {
    override private[theodolite] def apply(exchange: Exchange[C, D, C, D]): Exchange[C, D, S, T] =
      self.toExchange andThen other(exchange)

    override def review(d: D): T = self.review(other.review(d))
  }

  /** compose this [[AnIso_]] with an [[Iso_]], having this [[AnIso_]] applied last */
  final def compose[C, D](other: Iso_[C, D, S, T]): AnIso_[C, D, A, B] = new AnIso_[C, D, A, B] {
    override private[theodolite] def apply(exchange: Exchange[A, B, A, B]): Exchange[A, B, C, D] =
  ...
  final def andThen[C, D](other: Lens_[A, B, C, D]): Lens_[S, T, C, D] = new Lens_[S, T, C, D] {
    override private[theodolite] def apply[P[_, _]](pab: P[C, D])(implicit ev: Strong[P]): P[S, T] =
  ...
  final def andThen[C, D](other: Prism_[A, B, C, D]): Prism_[S, T, C, D] = new Prism_[S, T, C, D] {
    override private[theodolite] def apply[P[_, _]](pab: P[C, D])(implicit ev: Choice[P]): P[S, T] =
      ev.dimap(other(pab))(self.view)(self.review)
  ...
  final def andThen[C, D](other: APrism_[A, B, C, D]): APrism_[S, T, C, D] = new APrism_[S, T, C, D] {
    override private[theodolite] def apply(market: Market[C, D, C, D]): Market[C, D, S, T] = {
      val exchange: Exchange[A, B, S, T] = toExchange
      val marketFromExchange = Market(Right[T, A] _ compose exchange.view, exchange.review)

      marketFromExchange andThen other(market)
    }
  ...
  final def andThen[C, D](other: AnAffineTraversal_[A, B, C, D]): AnAffineTraversal_[S, T, C, D] = new AnAffineTraversal_[S, T, C, D] {
    override private[theodolite] def apply(pab: Stall[C, D, C, D]): Stall[C, D, S, T] =
  ...
  final def andThen[C, D](other: Traversal_[A, B, C, D]): Traversal_[S, T, C, D] = new Traversal_[S, T, C, D] {
    override private[theodolite] def apply[P[_, _]](pab: P[C, D])(implicit ev: Wander[P]): P[S, T] =
      ev.dimap(other(pab))(self.view)(self.review)
  }
  ...
  final def andThen[C, D](other: Setter_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    override private[theodolite] def apply(pab: C => D): S => T = self.over(other(pab))
  }
  ...
  final def compose[C, D](other: Setter_[C, D, S, T]): Setter_[C, D, A, B] = new Setter_[C, D, A, B] {
    override private[theodolite] def apply(pab: A => B): C => D = other(self.over(pab))
  }
  ...
  final def andThen[C, D](other: Getter_[A, B, C, D]): Getter_[S, T, C, D] = new Getter_[S, T, C, D] {
    override private[theodolite] def apply(forget: Forget[C, C, D]): Forget[C, S, T] =
  ...
  final def compose[C, D](other: Getter_[C, D, S, T]): Getter_[C, D, A, B] = new Getter_[C, D, A, B] {
    override private[theodolite] def apply(forget: Forget[A, A, B]): Forget[A, C, D] =
  ...
  final def compose[C, D](other: Grate_[C, D, S, T]): Grate_[C, D, A, B] = new Grate_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Closed[P]): P[C, D] =
  ...
  final def andThen[C, D](other: Review_[A, B, C, D]): Review_[S, T, C, D] = new Review_[S, T, C, D] {
    override private[theodolite] def apply(tagged: Tagged[C, D]): Tagged[S, T] =
  ...
  final def compose[C, D](other: Review_[C, D, S, T]): Review_[C, D, A, B] = new Review_[C, D, A, B] {
    override private[theodolite] def apply(tagged: Tagged[A, B]): Tagged[C, D] =
...
object AnIso_ {
  ...
  private[theodolite] def apply[S, T, A, B](f: Exchange[A, B, A, B] => Exchange[A, B, S, T]): AnIso_[S, T, A, B] = new AnIso_[S, T, A, B] { self =>
