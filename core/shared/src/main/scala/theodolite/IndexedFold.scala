...
import theodolite.indices.FoldableWithIndex
import theodolite.internal.{Forget, Indexed, IndexedFold0}
import theodolite.rank2types.Rank2TypeIndexedFoldLike
...
/** A [[IndexedFold_]] is a generalization of something Foldable. It describes how to retrieve multiple values and thier indices.
  ...
  *
  * An [[IndexedFold_]] is an indexed optic with fixed type [[theodolite.internal.Forget]] [[cats.arrow.Profunctor]]
  *
  * @tparam I
...
abstract class IndexedFold_[I, S, T, A, B] extends IndexedFold0[I, S, A] { self =>
  private[theodolite] def apply[R: Monoid](indexed: Indexed[Forget[R, *, *], I, A, B]): Forget[R, S, T]
  ...
  final def reindex[J](f: I => J): IndexedFold_[J, S, T, A, B] = new IndexedFold_[J, S, T, A, B] {
    override private[theodolite] def apply[R: Monoid](indexed: Indexed[Forget[R, *, *], J, A, B]): Forget[R, S, T] =
...
object IndexedFold_ {
  ...
  private[theodolite] def apply[I, S, T, A, B](f: Rank2TypeIndexedFoldLike[I, S, T, A, B])(implicit ev: DummyImplicit): IndexedFold_[I, S, T, A, B] =
  ...
  /** create a polymorphic [[IndexedFold_]] from [[theodolite.indices.FoldableWithIndex]] */
  final def fromFoldableWithIndex[F[_], I, A, B](implicit ev: FoldableWithIndex[F, I]): IndexedFold_[I, F[A], F[B], A, B] = new IndexedFold_[I, F[A], F[B], A, B] {
    override private[theodolite] def apply[R: Monoid](indexed: Indexed[Forget[R, *, *], I, A, B]): Forget[R, F[A], F[B]] =
...
object IndexedFold {
  ...
  /** create a monomorphic [[IndexedFold]] from [[theodolite.indices.FoldableWithIndex]] */
