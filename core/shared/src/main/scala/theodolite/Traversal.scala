...
import theodolite.Lens_.liftOptic
import theodolite.Traversal_.wander
...
import theodolite.rank2types.{LensLike, LensLikeWithIndex, Rank2TypeTraversalLike}
import theodolite.syntax.star._
import theodolite.syntax.traversal._
...
abstract class Traversal_[S, T, A, B] extends Traversal1[S, T, A, B] { self =>
  private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev: Wander[P]): P[S, T]
  ...
  /** convert a [[Traversal_]] to a [[theodolite.internal.Bazaar]] */
...
object Traversal_ {
  ...
  private[theodolite] def apply[S, T, A, B](lensLikeTraversal: Rank2TypeTraversalLike[S, T, A, B]): Traversal_[S, T, A, B] = new Traversal_[S, T, A, B] {
  ...
  /** create a polymorphic [[Traversal_]] from [[theodolite.internal.Bazaar]] */
...
object Traversal {
  ...
  /** create a monomorphic [[Traversal]] from a [[theodolite.internal.Bazaar]] */
