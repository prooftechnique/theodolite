...
import theodolite.internal.{Forget, Indexed, IndexedGetter1}
...
abstract class IndexedGetter_[I, S, T, A, B] extends IndexedGetter1[I, S, A] { self =>
  private[theodolite] def apply(indexed: Indexed[Forget[(A, I), *, *], I, A, B]): Forget[(A, I), S, T]
  ...
  final def reindex[J](f: I => J): IndexedGetter_[J, S, T, A, B] = new IndexedGetter_[J, S, T, A, B] {
    override private[theodolite] def apply(indexed: Indexed[Forget[(A, J), *, *], J, A, B]): Forget[(A, J), S, T] = {
  ...
  final def asGetter: Getter_[S, T, A, B] = new Getter_[S, T, A, B] {
    override private[theodolite] def apply(forget: Forget[A, A, B]): Forget[A, S, T] =
...
object IndexedGetter_ {
  ...
  private[theodolite] def apply[I, S, T, A, B](f: Indexed[Forget[(A, I), *, *], I, A, B] => Forget[(A, I), S, T])(implicit ev: DummyImplicit): IndexedGetter_[I, S, T, A, B] =
