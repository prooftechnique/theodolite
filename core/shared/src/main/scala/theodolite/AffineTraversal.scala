...
import theodolite.internal.{AffineTraversal0, Forget, Indexed, RunBazaar}
import theodolite.profunctor.{Choice, Star, Wander}
import theodolite.rank2types.{LensLikeWithIndex, Rank2TypeTraversalLike}
...
abstract class AffineTraversal_[S, T, A, B] extends AffineTraversal0[S, T, A, B] { self =>
  private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev0: Choice[P], ev1: Strong[P]): P[S, T]
  ...
  final def asFold: Fold_[S, T, A, B] = new Fold_[S, T, A, B] {
    final override private[theodolite] def apply[R: Monoid](forget: Forget[R, A, B]): Forget[R, S, T] =
  ...
  final def compose[C, D](other: Iso_[C, D, S, T]): AffineTraversal_[C, D, A, B] = new AffineTraversal_[C, D, A, B] {
    override private[theodolite] def apply[P[_, _]](pab: P[A, B])(implicit ev0: Choice[P], ev1: Strong[P]): P[C, D] = other(self(pab))(ev1)
  ...
  final def andThen[C, D](other: Lens_[A, B, C, D]): AffineTraversal_[S, T, C, D] = new AffineTraversal_[S, T, C, D] {
    final override private[theodolite] def apply[P[_, _]](pab: P[C, D])(implicit ev0: Choice[P], ev1: Strong[P]): P[S, T] = self(other(pab))
  ...
  final def andThen[C, D](other: Setter_[A, B, C, D]): Setter_[S, T, C, D] = new Setter_[S, T, C, D] {
    final override private[theodolite] def apply(pab: C => D): S => T = self(other(pab))
  }
  ...
  final def andThen[I, C, D](other: IndexedSetter_[I, A, B, C, D]): IndexedSetter_[I, S, T, C, D] = new IndexedSetter_[I, S, T, C, D] {
    final override private[theodolite] def apply(indexed: Indexed[* => *, I, C, D]): S => T =
  ...
  final def andThen[I, C, D](other: IndexedGetter_[I, A, B, C, D]): IndexedFold_[I, S, T, C, D] = new IndexedFold_[I, S, T, C, D] {
    final override private[theodolite] def apply[R: Monoid](indexed: Indexed[Forget[R, *, *], I, C, D]): Forget[R, S, T] =
