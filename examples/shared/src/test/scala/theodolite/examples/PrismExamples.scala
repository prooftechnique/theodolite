...
class PrismExamples extends TheodoliteSuite {
  ...
  test("check whether an option contains a specific value") {
    val hasTheodolite = Prism.only[Option[String]]("Theodolite".some)

    assertResult(None)(hasTheodolite.preview("other".some))
    assertResult(None)(hasTheodolite.preview(None))
    assertResult(Some(()))(hasTheodolite.preview("Theodolite".some))
  }

  test("is secured request") {
    val secure = prefixed[String, String]("https://")

    assertResult(None)(secure.preview("http://proofte.ch/Theodolite/"))
    assertResult("proofte.ch/Theodolite/".some)(secure.preview("https://proofte.ch/Theodolite/"))
  }
